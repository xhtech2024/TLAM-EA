import torch #使用import语句导入PyTorch包
import torch.nn as nn #将nn模块导入到当前命名空间，并将其别名为nn，可以使用nn.Module
import torch.nn.functional as F #包含torch.nn库中所有函数，同时包含大量loss和activation function
from torch.nn import Parameter
from highway import HighwayMLP
import math
from torch_geometric.nn import GATConv  # 图注意力层
from torch_geometric.nn.models import GAT  # 图注意力层
...
class GCN_alignment_cnn(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_num, adj, pre_trained_embedding, featureless=True):
        ...
        self.embedding = Parameter(torch.zeros([input_dim, hidden_dim], dtype=torch.float, requires_grad=True))
        nn.init.xavier_uniform_(self.embedding, gain=1)  # 原模型
        self.A = adj
        self.featureless = featureless
        self.dropout = 0.5
        self.dropout = nn.Dropout(self.dropout)
    ...
    def soft_attention(self, batch_x, batch_y, encoding, doc_att, att_words, use_cuda):
        ...
        for i in range(batch_size):
            wq, wa = align_weights(Q[i], A[i]) # 
            eq = weighted_encoding(A[i], wq)  # （11，200）
            ea = weighted_encoding(Q[i], wa)  # （13， 200）
            EQ.append(eq)
            EA.append(ea)
        return Q, A, EQ, EA,
    ...
    def forward(self, X, batch_x, batch_y, doc_att, att_words, use_cuda):
        if self.featureless != True:
            ...
            pre_matrix = self.A
        layer1 = torch.mm(pre_matrix,self.embedding)  # GCN 层 （18078，200） X

        layer2 = torch.mm(pre_matrix, layer1)
        #layer3 = torch.mm(pre_matrix, layer2)
        #layer4 = torch.mm(pre_matrix, layer3)

        Q, A, EQ, EA = self.soft_attention(batch_x, batch_y, layer2, doc_att, att_words, use_cuda)
        batch_size = len(batch_x)
        predict = []
        ...
        for i in range(batch_size):
            ...
            t = t.unsqueeze(c_idx)
            enc_outs = []
            for encoder in self.encoders:  # 调用CNN
                f_map = encoder(t)
                enc_ = F.relu(f_map)
                k_h = enc_.size()[h_idx]
...
#         self.conv1 = GATConv(in_feats, h_feats, heads=1, concat=False)
#         self.conv2 = GATConv(h_feats, out_feats, heads=1, concat=False)
...
class YGCN_alignment_cnn(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_num, adj, pre_trained_embedding, featureless=True):
        super(YGCN_alignment_cnn, self).__init__()
        self.embedding = pre_trained_embedding  # 使用 glove 词向量
        self.embedding.requires_grad = True
        self.embedding = Parameter(self.embedding)
        #self.embedding = Parameter(torch.zeros([input_dim, hidden_dim], dtype=torch.float, requires_grad=True))
        nn.init.xavier_uniform_(self.embedding, gain=1)
        ...
        self.A = adj
        self.gat2layer = GAT(in_channels=300,hidden_channels=hidden_dim,num_layers=2,out_channels=hidden_dim,dropout=0.5)
        coo_adj = sp.coo_matrix(adj.to('cpu'))
        values = coo_adj.data  
        indices = np.vstack((coo_adj.row, coo_adj.col))  # 我们真正需要的coo形式
        self.edge_idx = torch.LongTensor(indices).to('cuda:0')  # PyG框架需要的coo形式
    ...
    def forward(self, X, batch_x, batch_y, doc_att, att_words, use_cuda):
        ...
        layer  = self.gat2layer(self.embedding,self.edge_idx.to('cuda:0'))
        #layer2 = torch.mm(pre_matrix, layer1)
        #layer3 = torch.mm(pre_matrix, layer2)
        #layer4 = torch.mm(pre_matrix, layer3)

        Q, A, EQ, EA = self.soft_attention(batch_x, batch_y, layer, doc_att, att_words, use_cuda)
        batch_size = len(batch_x)
        predict = []

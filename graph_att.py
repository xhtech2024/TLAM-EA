...
datasets = ['BeerAdvo-RateBeer']
# build corpus
# dataset = sys.argv[1]
#dataset = 'DBLP-GoogleScholar'
#dataset = 'Amazon-Google'
dataset = 'BeerAdvo-RateBeer'
schema = [0,0,0,0] #0:compostional attribute 1:non-compositional or singular
...
### handle doc-att
att_type_mask = {}
previous_layer_length = len(all_doc)  # all_doc 实体数量
...
doc_att_embed = _assign_embedding(previous_layer_length+vocab_size, 300) # 200维词向量
#doc_att_embed = _assign_embedding(previous_layer_length+vocab_size, 200) # 200维词向量
node_size = previous_layer_length + vocab_size #  vocal
...
token_embed = _load_embedding(word_id_map,"Embedding/glove.6B.300d.txt", 300) # 从预训练的glove中抽取
#token_embed = _load_embedding(word_id_map,"Embedding/glove.6B.200d.txt", 200) # 从预训练的glove中抽取
#token_embed = load_fasttext(word_id_map,"Embedding/wiki-news-300d-1M.vec", 300)
node_size = previous_layer_length + vocab_size

embedding = torch.cat((doc_att_embed, token_embed), dim=0)  # 合并 

adj = sp.csr_matrix(
    (weight, (row, col)), shape=(node_size, node_size))  # 邻接矩阵 row是行索引，col是列索引 weight是对应的值
